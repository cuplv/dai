...
let pp fs {prev_start_line; prev_length;next_start_line;new_lines} = Format.fprintf fs "text diff: {prev_start_line:%i; prev_length:%i; next_start_line:%i; length(new_lines):%i" prev_start_line prev_length next_start_line (Array.length new_lines)
...
let btwn ~prev ~next =
  ...
  |> List.map ~f:(fun hunk ->
      ...
      | _ -> failwith "With ~context:0, patdiff should always yield 3-range hunks of form (Same [||]) :: r :: (Same [||]) :: []"
    )
