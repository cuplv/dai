...
val expr : Cfg.Loc.t -> CST.expression -> Ast.Expr.t * (Cfg.Loc.t * edge list)
...
val of_method_decl : Loc_map.t -> class_prefix:string -> CST.method_declaration -> (Loc_map.t * edge list * Cfg.Fn.t) option
(** construct a procedure's CFG from its declaration's concrete syntax tree *)

val edge_list_of_stmt_list : string -> Loc_map.t -> entry:Cfg.Loc.t -> exit:Cfg.Loc.t -> ret:Cfg.Loc.t -> CST.program -> Loc_map.t * edge list

val for_loop_header : string -> body_entry:Cfg.Loc.t -> body_exit:Cfg.Loc.t -> entry:Cfg.Loc.t -> exit:Cfg.Loc.t -> ret:Cfg.Loc.t -> Loc_map.t -> CST.for_statement -> Loc_map.t * edge list
