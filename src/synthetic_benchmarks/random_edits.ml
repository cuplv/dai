...
module Make (Dom : Abstract.Dom) = struct
  module G = Analysis.Dsg.Make (Dom)
  module D = G.D
  module N = Cfg.G.Node
  module E = Cfg.G.Edge
  module Nm = D.Name
...
*)
  ...
  let gen_stmt fn =
    ...
    | 0 -> Skip
    | 1 | 2 -> Assign { lhs = gen_bool_ident fn; rhs = gen_bool_expr fn }
    | 3 -> Assign { lhs = gen_array_ident fn; rhs = gen_array_literal fn }
  ...
  let add_call_at at_loc cfg (caller : Fn.t) dsg =
    ...
    let dsg =
      if Int.equal !functions (succ caller_idx) || Int.equal 0 (Random.int 5) then add_fn dsg
      else dsg
    in
    let callee_name =
    ...
    in
    let callee : Fn.t =
      List.find_exn (Fn.Map.keys dsg) ~f:(fun fn ->
          String.equal fn.method_id.method_name callee_name)
    in
  ...
  let random_query dsg =
    let _, dsg =
      ...
      let entrypoints = [ entrypoint dsg ] in
      G.loc_only_query ~fn ~loc ~cg ~fields ~entrypoints dsg
    in
    dsg
  ...
  let exit_query dsg =
    let _, dsg =
      ...
      let cg = !callgraph in
      let fields = Declared_fields.empty in
      G.loc_only_query ~fn ~loc ~cg ~fields ~entrypoints:[ fn ] dsg
    in
    dsg
