...
let analyze =
  basic ~summary:"Interactively analyze a Java program using the DAI framework."
    [%map_open
      ...
      and _batch = flag "batch" no_arg ~doc:"Run in batch mode, disabling interactivity" in
      fun () ->
        ...
        List.iter srcs ~f:(fun src ->
            ...
            Tree.parse ~old_tree:None ~file >>= Tree.as_java_cst file >>| Cfg_parser.of_java_cst
            |> function
